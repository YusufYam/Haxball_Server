...
export const freeKick = async (
  ...
  game.eventCounter += 1;
  const savedEventCounter = game.eventCounter;
  throwRealBall(game, forTeam, pos, savedEventCounter,1.2);
  const blockerId = forTeam == 1 ? 2 : 1;
  const notBlockerId = forTeam == 1 ? 1 : 2;
...
export const handleBallInPlay = async (game: Game) => {
  ...
  // 👇 DÜZELTME: 0.1 yerine 1.0 yaptık. Sadece sürtünmeyle dribbling açılmaz!
...
const throwRealBall = async (
  ...
  const bonus = customBonus !== undefined ? customBonus : 0.68;
  const toMass = game.rotateNextKick
    ? defaults.ballInvMass + bonus
...
export const penalty = async (
  ...
  game.eventCounter += 1;
  const savedEventCounter = game.eventCounter;
  throwRealBall(game, forTeam, pos, savedEventCounter, 0.68);
  room
    .getPlayerList()
    .filter((p) => p.team != 0)
    .forEach((p) => {
